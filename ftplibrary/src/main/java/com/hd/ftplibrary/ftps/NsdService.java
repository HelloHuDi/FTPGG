    ...
    public static class StartStopReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN) {
                Log.w(TAG, "onReceive: Running pre-JB, version to old for " + "NSD functionality, bailing out");
                return;
            }
            if (FsService.ACTION_STARTED.equals(intent.getAction())) {
                Intent service = new Intent(context, NsdService.class);
                context.startService(service);
            } else if (FsService.ACTION_STOPPED.equals(intent.getAction())) {
                Intent service = new Intent(context, NsdService.class);
